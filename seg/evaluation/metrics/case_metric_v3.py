...
class IoUMetric(BaseMetric):
    ...
    def compute_metrics(self, results: list) -> Dict[str, float]:
        ...
        for i, (case_name, slice_nums) in enumerate(case_nums.items()):
            ...
            case_pred = torch.concat(_results[1][start_slice:end_slice], 0)
            case_label = torch.concat(_results[2][start_slice:end_slice], 0)
            ret_metrics = self.label_to_metrics(
    ...
    @staticmethod
    def label_to_metrics(prediction: torch.Tensor, target: torch.Tensor,
                         num_classes: int, metrics: List[str], metric_funcs):
        ...
        #     nproc=num_classes - 1)
        rets = []
        post = AsDiscrete(to_onehot=num_classes)
        for metric_func in metric_funcs:
            # ret = ALL_METRICS[metric](pred, gt, confusion_matrix)
            ret = metric_func(
                y_pred=post(prediction.unsqueeze(0)).unsqueeze(0),
                y=post(target.unsqueeze(0)).unsqueeze(0))
            rets.append(ret)

        # results = list(zip(*ret))

        ret_metrics = OrderedDict({
            metric_name: rets[i].squeeze().cpu().numpy()
